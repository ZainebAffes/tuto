    ...
    toRaw: function (formattedData, model) {
        ...
        if (formattedData < 0) {
            ...
            if (parseFloat(prixpublic) < parseFloat(formattedData)) {
                ...
                model.set('majoration', parseFloat(isFinite(prix_conv) ? prix_conv : 0).toFixed(2));
                return formattedData;
            } else {
                model.set('majoration', "0.00");
                var prix_conv = (1 - (formattedData / prixpublic)) * 100;
                model.set('remise', parseFloat(isFinite(prix_conv) ? prix_conv : 0).toFixed(2));
                return formattedData;
            }
        } else
            return  formattedData;
    ...
    toRaw: function (formattedData, model) {
        ...
        if (formattedData < 0) {
            ...
            if (parseFloat(prixhonoraire) < parseFloat(formattedData)) {
                ...
                model.set('majorationhonoraire', parseFloat(isFinite(prix_conv) ? prix_conv : 0).toFixed(2));
                return formattedData;
            } else {
                model.set('majorationhonoraire', "0.00");
                var prix_conv = (1 - (formattedData / prixhonoraire)) * 100;
                model.set('remisehonoraire', parseFloat(isFinite(prix_conv) ? prix_conv : 0).toFixed(2));
                return formattedData;
            }
        } else {
            return  formattedData;
...
var Select2ListVille = Backgrid.Select2Cell.extend({
    select2Options: function (model) {
        var response = [];
        $.ajax({
            url: url_base + '/souregfrs/filter?codGouv=' + model.get("codregion") + '&actifs=true',
