...
let url_menu = "/workflowDemande-web/menu";

$(document).ready(function (e) {
    let xhr = new XMLHttpRequest();
    xhr.open("GET", `${url_base_fichier_base}/utilisateur/is-authenticated`);
    xhr.responseType = "json";
    xhr.setRequestHeader("Content-type", "application/json");
    xhr.setRequestHeader("x-auth-token", localStorage.getItem("x-auth-token"));
    xhr.onload = function () {
        if (xhr.status === 200 && xhr.response !== null) {
            window.location.href = "/workflowDemande-web/reconnect";
        }
    };
    xhr.send(JSON.stringify());
    NodeList.prototype.forEach = Array.prototype.forEach;
    if (window.sessionStorage) {
        $("#username").val(window.localStorage.getItem('loginFichierBase'));
        $("#password").val(window.localStorage.getItem('passwordFichierBase'));
    }
