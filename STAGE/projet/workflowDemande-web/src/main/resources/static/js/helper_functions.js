...
function findParamByCode(code) {
    var response = "";
    var url =  `${url_base_fichier_base}/params?code=`+ code;
...
$(document).ajaxError(function myErrorHandler(event, xhr, ajaxOptions, thrownError) {
    if (xhr.status === 409) {
            ...
            $("#cancelAuthentification").bind("click", function (e) {
                ...
                xhr.onload = function () {
                    if (xhr.status === 200) {
                        ...
                        document.cookie = "";
                        window.location.href = "/workflowDemande-web/";
                    }
                };
        ...
        $("#cancelSessionExpiration").bind("click", function (e) {
            window.parent.location.href = "/workflowDemande-web/login";
        });
...
function findSocieteByCode() {
    ...
    var response = "";
    $.ajax({
        url: `${url_base}/societes/${Societe}?lazy=true`,
        type: 'GET',
        async: false,
        dataType: 'json',
        success: function (data)
...
function findExerciceByExerciceCodeSoc() {
    ...
    var response = "";
    $.ajax({
        url: `${url_base}/exercice/find-exercice?exercice=${Exercice}&codSoc=${Societe}`,
        headers: {
            'x-auth-token': localStorage.getItem("x-auth-token"),
            'Accept-Language': localStorage.getItem("langue")
        },
...
function findClotureMoisByCodeSocExerciceMois(mois) {
    ...
    var response = null;
    $.ajax({
        url: url_base + '/cloturemois/findByCodeSocAndAnneeAndMois?annee=' + Exercice + '&codeSoc=' + Societe + '&mois=' + mois + '&cloturer=true&definitive=true,false',
        type: 'GET',
        async: false,
        dataType: 'json',
        success: function (data)
...
function findJournauxCodJouCodeSoc(codJou) {
    ...
    var response = "";
    $.ajax({
        url: `${url_base}/journaux/getJournal?codJou=${codJou}&codeSoc=${Societe}`,
        type: 'GET',
        async: false,
        dataType: 'json',
        success: function (data)
...
function getDateTime() {
    var response = "";
    $.ajax({
        url: `${url_base}/server-date/date-time`,
        type: 'GET',
        async: false,
        dataType: 'json',
        success: function (data)
...
function getTraceNotification() {
    ...
    for (var i = 0; i < listTr.length; i++) {
        ...
        html += '                                <span class="subject"><strong>N° liste patient:</strong>&nbsp&nbsp' + listTr[i].tracePK.numCha + '<i class="icon-paperclip"></i></span>';
